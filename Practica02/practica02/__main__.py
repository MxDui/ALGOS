...
def print_steps(steps: List[List[int]]) -> None:
    """
    Imprime los pasos de ordenamiento en un formato legible.
    
    Args:
        steps: Lista de listas, cada una representando un paso en el proceso de ordenamiento
    """
    # No mostrar todos los pasos para listas grandes para evitar salida abrumadora
    total_steps = len(steps)
    
    # Muestra un número razonable de pasos
    if total_steps <= 2:
        ...
        # Para listas pequeñas, mostramos todos los pasos
        if total_steps <= 10:
            for i in range(1, total_steps - 1):
                print(f"Paso {i}: {steps[i]}")
        else:
            # Para recuentos de pasos más grandes, mostramos ~8 pasos distribuidos uniformemente
            ...
            # Nos aseguramos de mostrar el último paso intermedio si aún no lo hemos hecho
...
def run_demo(size: int, trace: bool, seed: Optional[int], use_linked_list: bool = False) -> None:
    """
    Ejecuta una demostración de todos los algoritmos de ordenamiento.
    ...
    print(f"\n===== DEMOSTRACIÓN DE PRACTICA02 (Implementación con {implementation}) =====\n")
    ...
    for algorithm_name in list_algorithms():
        ...
        print(f"\n----- Ordenando con {algorithm_name.capitalize()} Sort -----")
        print(f"Entrada: {data}")
        
        if trace:
            start_time = time.perf_counter()
            if use_linked_list:
                # Creamos una nueva lista enlazada para cada algoritmo
                algo_input = DoublyLinkedList(data)
                steps = list(algorithm(algo_input, trace=True))
                # Convertimos los pasos de la lista enlazada a listas regulares para la visualización
                list_steps = [step.to_list() for step in steps]
            else:
...
def main():
    # Usamos un tamaño pequeño para mostrar mejor los pasos del algoritmo
    size = 8
    trace = True
    seed = 42
    
    # Ejecuta con la implementación de array
    run_demo(size=size, trace=trace, seed=seed, use_linked_list=False)
    
    # Ejecuta con la implementación de lista enlazada
